...
def validate_completed_gate(gate: Gate, run: Mapping[str, object], source_sha: str) -> None:
    ...
    if failures:
        ...
        raise GateError(f"{gate.workflow_name} run {run_id} blocked publication: {joined}")
    ...
    for attempt in range(1, discovery_attempts + 1):
        for gate in GATES:
            if gate.key in selected:
                continue
            runs = client.list_runs(gate.workflow_file, source_sha)
            match = select_latest_exact_run(runs, source_sha)
...
def main() -> int:
    ...
    runs = prove_release_gates(
        ...
        discovery_sleep_seconds=_nonnegative_float(
            "RELEASE_GATE_DISCOVERY_SLEEP_SECONDS", 15.0
        ),
