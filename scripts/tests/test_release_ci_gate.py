...
def run_for(gate, run_id, *, sha=SHA, created_at="2026-08-10T10:00:00Z", status="completed", conclusion="success", job_overrides=None):
...
class ReleaseGateTests(unittest.TestCase):
    ...
    def test_integration_red_blocks_publication(self):
        ...
        views[1] = failed
        with self.assertRaisesRegex(GateError, "Tauri migration integration.*blocked publication"):
    ...
    def test_windows_package_red_blocks_publication(self):
        ...
        listed[gate.workflow_file] = [failed]
        views[3] = failed
        with self.assertRaisesRegex(GateError, "Windows Tauri Installer.*blocked publication"):
            prove(FakeClient(listed, views))
    ...
    def test_wrong_sha_run_is_ignored(self):
        ...
        views[99] = wrong
        with self.assertRaisesRegex(GateError, "Missing mandatory.*Tauri migration integration"):
            prove(FakeClient(listed, views))

    def test_stale_historical_green_run_is_ignored_in_favor_of_latest_exact_sha(self):
        listed, views = healthy_fixture()
        gate = next(g for g in GATES if g.key == "integration")
        historical_green = run_for(gate, 90, created_at="2026-08-10T09:00:00Z")
        latest_failed = run_for(gate, 91, created_at="2026-08-10T10:00:00Z", conclusion="failure")
    ...
    def test_skipped_mandatory_job_is_not_success(self):
        ...
        skipped = run_for(gate, 1, job_overrides={skipped_name: job(skipped_name, conclusion="skipped")})
    ...
    def test_in_progress_run_is_waited_for_then_accepted(self):
        ...
        sleeps = []
        result = prove(FakeClient(listed, views), wait_attempts=2, sleep=sleeps.append)
