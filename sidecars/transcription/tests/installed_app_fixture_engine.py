...
def write_artifacts(job: dict[str, object]) -> tuple[Path, dict[str, str]]:
    ...
    artifacts["timestamp_text"].write_text(f"[00:00.000 --> 00:01.750] {TRANSCRIPT_TEXT}\n", encoding="utf-8")
    artifacts["srt"].write_text(f"1\n00:00:00,000 --> 00:00:01,750\n{TRANSCRIPT_TEXT}\n", encoding="utf-8")
    artifacts["vtt"].write_text(f"WEBVTT\n\n00:00.000 --> 00:01.750\n{TRANSCRIPT_TEXT}\n", encoding="utf-8")
    artifacts["json"].write_text(
        json.dumps({"text": TRANSCRIPT_TEXT, "language": "en", "duration_seconds": 1.75}, separators=(",", ":")) + "\n",
        encoding="utf-8",
    )
...
def result_for(job: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "transcript": {
            ...
            "segments": [{"index": 1, "start": 0.0, "end": 1.75, "text": TRANSCRIPT_TEXT, "words": words}],
            "words": words,
            "language": "en",
...
def main() -> int:
    ...
    for line in sys.stdin:
        ...
        emit("progress", job_id=job_id, stage="transcribing", percent=40.0, message="deterministic fixture transcription")
        emit("progress", job_id=job_id, stage="processing", percent=90.0, message="writing deterministic fixture artifacts")
