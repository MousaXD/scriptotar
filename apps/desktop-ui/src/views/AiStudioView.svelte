...
<div class="ai-mode-card panel">
  ...
  {#if mode === 'byok'}<div class="byok-strip" data-testid="byok-fields"><label><span>API key · session only</span><input aria-label="API key" type="password" bind:value={apiKey} autocomplete="off" placeholder="Paste key for this run" /></label>{#if provider === 'OpenAI-compatible'}<label><span>Base URL</span><input bind:value={baseUrl} placeholder="https://…" /></label>{/if}<p>Keys are not persisted by this frontend. The Rust host will own secure storage and endpoint policy.</p></div>{/if}
</div>
