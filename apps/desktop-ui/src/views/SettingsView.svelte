...
<div class="settings-layout" aria-busy={busy || migrationBusy}>
  ...
  <div class="settings-stack">
    <section id="settings-transcription" class="panel settings-section"><div><span class="eyebrow">Transcription</span><h2>Speech engine</h2><p>Choose quality and compute defaults. Model installation remains a host responsibility.</p></div><div class="settings-grid"><label><span>Whisper model</span><select bind:value={draft.whisperModel}><option>small</option><option>medium</option><option>turbo</option><option>large-v3</option></select></label><label><span>Device</span><select bind:value={draft.device}><option>auto</option><option>cpu</option><option>cuda</option></select></label><label><span>Language</span><select bind:value={draft.language}><option>auto</option><option>Arabic</option><option>English</option></select></label></div></section>

    <section id="settings-media" class="panel settings-section"><div><span class="eyebrow">Downloads + cookies</span><h2>Media acquisition</h2><p>Browser cookies are selected by browser name only. Cookie secrets never belong in the frontend.</p></div><div class="settings-grid"><label><span>Quality</span><select bind:value={draft.quality}><option>720p</option><option>1080p</option><option>Best</option><option>Audio only</option></select></label><label><span>Browser cookies</span><select bind:value={draft.cookies}><option>none</option><option>firefox</option><option>chrome</option><option>chromium</option><option>brave</option></select></label><label><span>Duration safety limit</span><select bind:value={draft.maxDuration}><option>30 min</option><option>60 min</option><option>2 hours</option><option>Unlimited</option></select></label></div></section>

    <section id="settings-storage" class="panel settings-section"><div><span class="eyebrow">Storage</span><h2>Transcript output</h2><p>Choose where new transcription result folders are created. Rust verifies that a selected directory exists and is writable before saving it.</p></div><div class="output-picker"><span class="field-label">Current output directory</span><code>{draft.outputDirectory || 'Application default'}</code><div class="output-actions"><button class="button secondary" disabled={busy} on:click={chooseOutputDirectory}>Choose output folder</button><button class="button secondary" disabled={busy || draft.outputDirectory === null} on:click={restoreDefaultOutput}>Restore default</button></div></div></section>

    <section id="settings-local" class="panel settings-section"><div><span class="eyebrow">Privacy + processing</span><h2>Local behavior</h2><p>These values are persisted by the Rust settings layer and applied to future jobs.</p></div><div class="toggle-grid"><label><input type="checkbox" bind:checked={draft.copyLocalSource}/><span><strong>Copy local source media</strong><small>Keep a copy beside generated transcript artifacts.</small></span></label><label><input type="checkbox" bind:checked={draft.translate}/><span><strong>Translate speech to English</strong><small>Ask the transcription worker to translate.</small></span></label><label><input type="checkbox" bind:checked={draft.batched}/><span><strong>Batched inference</strong><small>Faster on suitable hardware, with higher memory use.</small></span></label><label><input type="checkbox" bind:checked={draft.keepFailed}/><span><strong>Keep failed partial artifacts</strong><small>Useful for debugging interrupted media stages.</small></span></label></div></section>

    <section id="settings-watchlists" class="panel settings-section">
    ...
    <section id="settings-migration" class="panel settings-section migration-section">
...
{#if status}<p class="status-copy settings-status" aria-live="polite">{status}</p>{/if}
{#if saveError}<p class="status-copy settings-status" role="alert">{saveError}</p>{/if}

<style>
  .settings-head { align-items: center; }
  .save-cluster { display: flex; align-items: center; gap: 10px; }
  .dirty-indicator { display: inline-flex; align-items: center; gap: 6px; color: var(--muted); font-size: 10px; white-space: nowrap; }
  .dirty-indicator > span { width: 6px; height: 6px; border-radius: 50%; background: var(--warn); }
  .settings-layout { display: grid; grid-template-columns: 190px minmax(0, 1fr); gap: 12px; align-items: start; }
  .settings-nav { position: sticky; top: 82px; display: grid; gap: 2px; padding: 8px; }
  .settings-nav-title { padding: 8px 9px 5px; color: var(--faint); font-size: 9px; font-weight: 700; letter-spacing: .07em; text-transform: uppercase; }
  .settings-nav-title.secondary-group { margin-top: 7px; padding-top: 11px; border-top: 1px solid var(--border); }
  .settings-nav a { min-height: 34px; display: flex; align-items: center; padding: 0 9px; border-radius: 7px; color: var(--muted); text-decoration: none; font-size: 11px; }
  .settings-nav a:hover, .settings-nav a:focus-visible { color: var(--text); background: var(--surface-2); }
  .settings-stack { display: grid; gap: 12px; min-width: 0; }
  .settings-section { scroll-margin-top: 82px; grid-template-columns: minmax(220px, 280px) minmax(0, 1fr); }
  .settings-section > div:first-child { max-width: 310px; }
  .output-picker { display: grid; gap: 9px; min-width: 0; }
  .field-label { color: var(--muted); font-size: 10px; }
  code { display: block; max-width: 560px; overflow-wrap: anywhere; padding: 10px 12px; border: 1px solid var(--border); border-radius: 9px; background: var(--surface-2); color: var(--text); }
  ...
  .migration-section { border-color: color-mix(in srgb, var(--info) 18%, var(--border)); }
  .migration-status { display: grid; gap: 10px; min-width: min(600px, 100%); padding: 14px; border: 1px solid var(--border); border-radius: 10px; background: color-mix(in srgb, var(--surface-2) 42%, transparent); }
  ...
  .settings-status { margin-inline-start: 202px; }

  @media (max-width: 1000px) {
    .settings-layout { grid-template-columns: 1fr; }
    .settings-nav { position: static; display: flex; gap: 4px; overflow-x: auto; }
    .settings-nav-title { display: none; }
    .settings-nav a { flex: 0 0 auto; border: 1px solid var(--border); background: var(--surface); }
    .settings-status { margin-inline-start: 0; }
  }
