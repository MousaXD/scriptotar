...
describe('Tauri client command contract', () => {
  it('uses the Rust command names and camelCase argument names exactly', async () => {
    ...
    expect(invokeMock).toHaveBeenNthCalledWith(7, 'scan_creator', { query: { profileUrl: 'https://www.youtube.com/@fixture', limit: 25 } });
    expect(invokeMock).toHaveBeenNthCalledWith(8, 'queue_research', { ids: ['research-id'] });
    expect(invokeMock).toHaveBeenNthCalledWith(9, 'cancel_job', { jobId: 'job-id' });
    expect(invokeMock).toHaveBeenNthCalledWith(10, 'build_ai_prompt', { input: aiInput });
    expect(invokeMock).toHaveBeenNthCalledWith(11, 'run_ai', { input: aiInput });
    expect(invokeMock).toHaveBeenNthCalledWith(12, 'get_settings');
    expect(invokeMock).toHaveBeenNthCalledWith(13, 'save_settings', { settings: expect.objectContaining({ whisperModel: 'medium' }) });
    expect(invokeMock).toHaveBeenNthCalledWith(14, 'import_legacy_data');
  });
});
