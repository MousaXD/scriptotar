...
pub struct AppServices {
    ...
    _watchlist_worker: Arc<WatchlistWorker>,
}
...
impl AppServices {
    pub fn new(data_dir: impl AsRef<Path>) -> RepositoryResult<Self> {
        ...
        let watchlist_worker = spawn_watchlist_refresher(store.clone(), research_command.clone());
        ...
        Ok(Self {
            ...
            legacy_path,
            research_command,
            _watchlist_worker: watchlist_worker,
        })
    }
...
fn spawn_watchlist_refresher(store: SqliteStore, command: YtDlpCommand) -> Arc<WatchlistWorker> {
    let (stop_sender, stop_receiver) = mpsc::channel();
    let handle = thread::spawn(move || loop {
        match stop_receiver.recv_timeout(WATCHLIST_TICK) {
...
mod tests {
    ...
    #[test]
    fn list_jobs_queries_only_the_active_project() {
        let temp = tempdir().unwrap();
        let services = AppServices::new(temp.path()).unwrap();
        let active_project = services.active_project_id().unwrap();
        let other_project = Project::new("Other");
        services.store.create_project(&other_project).unwrap();
        let active_job = JobService::new(services.store.clone())
            .enqueue(
                active_project,
                JobInput::Url("https://www.youtube.com/watch?v=active".to_owned()),
            )
            .unwrap();
        JobService::new(services.store.clone())
            .enqueue(
                other_project.id,
                JobInput::Url("https://www.youtube.com/watch?v=other".to_owned()),
            )
            .unwrap();

        let jobs = services.list_jobs().unwrap();
        assert_eq!(jobs.len(), 1);
        assert_eq!(jobs[0].id, active_job.id.to_string());
    }

    #[test]
    fn watchlist_worker_stops_when_owner_is_dropped() {
        let temp = tempdir().unwrap();
        let store = SqliteStore::open(temp.path().join("watch.sqlite3")).unwrap();
        store.run_integration_migrations().unwrap();
        let worker = spawn_watchlist_refresher(store, YtDlpCommand::executable("unused"));
        drop(worker);
    }
